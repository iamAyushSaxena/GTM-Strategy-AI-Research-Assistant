...
def load_data():
    data = {}
    try:
        data['competitors'] = pd.read_csv(PROCESSED_DATA_DIR / 'competitive_overview.csv')
        ...
        data['financial'] = pd.read_csv(PROCESSED_DATA_DIR / 'financial_projections_24m.csv')
...
if selected_page == "Executive Summary":
    st.markdown('<h1 class="main-header">🚀 Go-to-Market Strategy</h1>', unsafe_allow_html=True)
    st.markdown(f'<h2 style="text-align: center; color: #6366f1;">{PRODUCT_NAME}</h2>', unsafe_allow_html=True)
    st.markdown(f'<h3 style="text-align: center; color: #8b5cf6; font-style: italic;">{PRODUCT_TAGLINE}</h3>', unsafe_allow_html=True)
    ...
    **Market Inefficiency:** Existing AI productivity tools either:
    ...
    # 90-Day Plan Summary
    st.markdown("")
    st.markdown('<p class="sub-header">📅 90-Day Launch Plan</p>', unsafe_allow_html=True)
    
    tab1, tab2, tab3 = st.tabs(["Phase 1: Beta (Days 1-30)", "Phase 2: Launch (Days 31-60)", "Phase 3: Scale (Days 61-90)"])
    ...
    st.markdown('<h1 class="main-header">🎯 Market Opportunity</h1>', unsafe_allow_html=True)
    
    # TAM/SAM/SOM
    st.markdown('<p class="sub-header">📊 Market Sizing (TAM/SAM/SOM)</p>', unsafe_allow_html=True)
    ...
    # Positioning Matrix
    st.markdown('<p class="sub-header">📍 Competitive Positioning</p>', unsafe_allow_html=True)
    ...
    # Competitor Overview
    st.markdown('<p class="sub-header">📊 Competitor Overview</p>', unsafe_allow_html=True)
    
    comp_display = data['competitors'][['name', 'positioning', 'estimated_users', 
                                         'pricing_lowest', 'total_funding_usd']].copy()
    ...
    comp_display['Funding'] = comp_display['Funding'].apply(lambda x: f"${x/1_000_000:.1f}M" if x > 0 else "Bootstrapped")
...
#    st.markdown("---")
    ...
    st.markdown("")
    st.markdown("")
    st.markdown('<p class="sub-header">💪 SWOT Analysis (Our Product)</p>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.markdown("")
        st.markdown("\n**Opportunities:**")
        opportunities = our_swot[our_swot['category'] == 'Opportunities']['factor'].tolist()
    ...
    st.markdown('<h1 class="main-header">💰 Pricing Strategy</h1>', unsafe_allow_html=True)
    
    # Proposed Pricing
    st.markdown('<p class="sub-header">💵 Proposed Pricing Tiers</p>', unsafe_allow_html=True)
    ...
    # Competitive Pricing
    st.markdown('<p class="sub-header">📊 Competitive Pricing Landscape</p>', unsafe_allow_html=True)
    
    # Create pricing comparison data
    pricing_comp = pd.DataFrame([
        {'Product': 'Our Product (Pro)', 'Price': 15, 'Positioning': 'Specialist', 'Highlight': True},
        {'Product': 'Notion AI', 'Price': 10, 'Positioning': 'Generalist', 'Highlight': False},
        {'Product': 'Mem.ai', 'Price': 8, 'Positioning': 'Generalist', 'Highlight': False},
        {'Product': 'Reflect', 'Price': 10, 'Positioning': 'Generalist', 'Highlight': False},
        {'Product': 'Roam Research', 'Price': 15, 'Positioning': 'Generalist', 'Highlight': False},
        {'Product': 'Recall', 'Price': 7, 'Positioning': 'Specialist', 'Highlight': False}
    ])
...
#    st.markdown("---")
    ...
    with col1:
        st.metric("Customer Acquisition Cost (CAC)", "$35", delta="Blended across channels")
    ...
    with col3:
        ...
        st.metric("Payback Period", "3.3 months", delta="-8.7mo vs target", delta_color="inverse")
    ...
    st.markdown('<h1 class="main-header">📅 90-Day GTM Plan</h1>', unsafe_allow_html=True)
    ...
    phase_filter = st.selectbox(
        "Filter by Phase:",
        ["All Phases", "Phase 1: Private Beta", "Phase 2: Public Launch", "Phase 3: Paid Acquisition"]
    )
    ...
    for _, week in filtered_plan.iterrows():
        with st.expander(f"**Week {week['week']}: {week['focus']}** ({week['start_date']} to {week['end_date']})"):
    ...
    # Budget Allocation
    st.markdown('<p class="sub-header">💵 Budget Allocation</p>', unsafe_allow_html=True)
    
    # 1. Define Data
    budget_data = pd.DataFrame([
        {'Phase': 'Month 1: Foundation', 'Amount': 5000, 'Focus': 'MVP & Setup'},
        {'Phase': 'Month 2: Validation', 'Amount': 15000, 'Focus': 'Paid Ads & Content'},
    ...
    with col1:
        ...
        fig = go.Figure(data=[go.Pie(
            ...
            textposition='inside',
            marker=dict(colors=[COLOR_SCHEME['secondary'], COLOR_SCHEME['primary'], COLOR_SCHEME['accent']]),
    ...
    # Channel Strategy
    st.markdown('<p class="sub-header">📡 Acquisition Channels</p>', unsafe_allow_html=True)
    
    channels_data = pd.DataFrame([
        {'Channel': 'Product Hunt', 'Users': 800, 'CAC': 20, 'Budget': 5000, 'Type': 'Launch'},
        {'Channel': 'SEO/Content', 'Users': 1200, 'CAC': 15, 'Budget': 10000, 'Type': 'Organic'},
        {'Channel': 'Google Ads', 'Users': 1500, 'CAC': 50, 'Budget': 25000, 'Type': 'Paid'},
        {'Channel': 'Referrals', 'Users': 600, 'CAC': 10, 'Budget': 3000, 'Type': 'Viral'},
        {'Channel': 'Social Media', 'Users': 500, 'CAC': 25, 'Budget': 8000, 'Type': 'Community'},
        {'Channel': 'Partnerships', 'Users': 400, 'CAC': 30, 'Budget': 4000, 'Type': 'B2B'}
    ])
    ...
    st.markdown('<h1 class="main-header">💵 Financial Projections</h1>', unsafe_allow_html=True)
    
    # Interactive projections
    st.markdown('<p class="sub-header">📈 24-Month Projections</p>', unsafe_allow_html=True)
...
"""
    ...
    with c2:
        st.markdown(create_milestone_card(6, "Scaling Phase", m6), unsafe_allow_html=True)
    with c3:
        st.markdown(create_milestone_card(12, "Year 1 End", m12), unsafe_allow_html=True)
    with c4:
        st.markdown(create_milestone_card(24, "Year 2 End", m24), unsafe_allow_html=True)
    
    st.markdown("---")
    
    # Detailed table
    st.markdown('<p class="sub-header">📊 Detailed Projections</p>', unsafe_allow_html=True)
    st.markdown("24 Months of financial projections including user growth, revenue, and key metrics:")
    
    # Select columns to display
    display_cols = ['month', 'date', 'total_users', 'paying_users', 'mrr', 'arr', 
                   'free_to_paid_rate', 'churn_rate', 'ltv_cac_ratio', 'net_profit']
    
    display_df = data['financial'][display_cols].copy()
    display_df.columns = ['Month', 'Date', 'Total Users', 'Paying Users', 'MRR', 'ARR',
                          'Free→Paid %', 'Churn %', 'LTV/CAC', 'Net Profit']
    ...
    display_df['Free→Paid %'] = display_df['Free→Paid %'].apply(lambda x: f"{x*100:.1f}%")
    ...
    with col1:
        st.metric("TAM", f"{MARKET_SIZE['tam']['size']/1_000_000:.0f}M", "Knowledge Workers")
    
    with col2:
        st.metric("SAM", f"{MARKET_SIZE['sam']['size']/1_000_000:.0f}M", f"{MARKET_SIZE['sam']['penetration']*100:.0f}% of TAM")
    
    with col3:
        st.metric("SOM", f"{MARKET_SIZE['som']['size']/1_000_000:.1f}M", f"{MARKET_SIZE['som']['penetration']*100:.0f}% of SAM")
...
#    st.markdown("---")
    ...
    st.markdown("")
    st.markdown("")
    st.markdown('<p class="sub-header">🎯 Key Strategic Insights</p>', unsafe_allow_html=True)
