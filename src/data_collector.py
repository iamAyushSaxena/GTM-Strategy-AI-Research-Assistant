...
class CompetitiveDataCollector:
    ...
    def generate_feature_matrix(self) -> pd.DataFrame:
        ...
        for competitor_id, scores in feature_scores.items():
            for feature, score in scores.items():
                matrix_data.append(
                    {"competitor_id": competitor_id, "feature": feature, "score": score}
                )

        self.features_matrix = pd.DataFrame(matrix_data)

        print(
            f"✅ Generated feature matrix: {len(FEATURE_DIMENSIONS)} features × {len(feature_scores)} products"
        )
    ...
    def save_all_data(self):
        ...
        if self.competitors_df is not None:
            self.competitors_df.to_csv(
                PROCESSED_DATA_DIR / "competitive_overview.csv", index=False
            )
            print(f"💾 Saved: competitive_overview.csv")

        if self.features_matrix is not None:
            self.features_matrix.to_csv(
                PROCESSED_DATA_DIR / "feature_matrix.csv", index=False
            )
            print(f"💾 Saved: feature_matrix.csv")

        if self.traffic_data is not None:
            self.traffic_data.to_csv(
                SYNTHETIC_DATA_DIR / "traffic_estimates.csv", index=False
            )
            print(f"💾 Saved: traffic_estimates.csv")

        if self.reviews_data is not None:
            self.reviews_data.to_csv(
                SYNTHETIC_DATA_DIR / "user_reviews.csv", index=False
            )
