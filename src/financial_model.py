...
class FinancialModel:
    ...
    def project_user_growth(self, months: int = 24) -> pd.DataFrame:
        ...
        for month in range(1, months + 1):
            ...
            if month <= 3:
                ...
                free_to_paid_rate = min(
                    0.10, 0.08 + ((month - 3) * 0.003)
                )  # Approaching 10%

            new_paying = int(total_new_users * free_to_paid_rate)
            churned_paying = int(
                current_paying * churn_rate * 0.5
            )  # Paying users churn less
            ...
            projections.append(
                {
                    "month": month,
                    "date": (datetime.now() + timedelta(days=30 * month)).strftime(
                        "%Y-%m"
                    ),
        ...
        print(
            f"   Month 12: {self.monthly_projections.iloc[11]['total_users']:,.0f} users, ${self.monthly_projections.iloc[11]['mrr']:,.0f} MRR"
        )
        print(
            f"   Month 24: {self.monthly_projections.iloc[23]['total_users']:,.0f} users, ${self.monthly_projections.iloc[23]['mrr']:,.0f} MRR"
        )
    ...
    def generate_financial_report(self) -> str:
        ...
        if self.profitability_analysis["reached"]:
            ...
            report += (
                f"Note: May need to adjust operating costs or improve unit economics\n"
            )
        ...
        total_costs_24m = (
            self.monthly_projections["total_cogs"] + self.monthly_projections["opex"]
        ).sum()
        ...
        user_growth_rate = (
            ((m24_users / m1_users) ** (1 / 23) - 1) if m1_users > 0 else 0
        )
        ...
        if m12["gross_margin"] >= 0.70:
            ...
            report += (
                f"   ❌ Gross margin > 70% (current: {m12['gross_margin']*100:.1f}%)\n"
            )
