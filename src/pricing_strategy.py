...
class PricingStrategy:
    ...
    def analyze_competitor_pricing(self, competitors_df: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"   Average paid tier: ${self.competitor_pricing[self.competitor_pricing['price_monthly'] > 0]['price_monthly'].mean():.2f}/month")
        print(f"   Median paid tier: ${self.competitor_pricing[self.competitor_pricing['price_monthly'] > 0]['price_monthly'].median():.2f}/month")
        print(f"   Range: ${self.competitor_pricing[self.competitor_pricing['price_monthly'] > 0]['price_monthly'].min():.0f} - ${self.competitor_pricing[self.competitor_pricing['price_monthly'] > 0]['price_monthly'].max():.0f}")
    ...
    def calculate_value_metrics(self) -> Dict:
        ...
        roi_analysis = {
            ...
            'monthly_roi': (time_savings['monthly_value_saved'] - our_pricing['monthly_cost']) / our_pricing['monthly_cost'],
            'payback_hours': our_pricing['monthly_cost'] / 50,  # Hours of work to pay for tool
        ...
        print(f"   Monthly value delivered: ${roi_analysis['monthly_value_delivered']:.0f}")
    ...
    def validate_unit_economics(self, assumptions: Dict = None) -> Dict:
        ...
        costs = {
            'cogs': {
                'openai_api_per_user': assumptions.get('api_cost', 3.50),  # $3.50/user/month (GPT-4)
                'infrastructure_per_user': assumptions.get('infra_cost', 0.50),  # AWS, storage
                'total_per_user': 4.00
            },
        ...
        customer_economics = {
            ...
            'gross_profit_per_user': UNIT_ECONOMICS['ltv']['arpu'] * costs['gross_margin'],
            'retention_rate_monthly': assumptions.get('retention', 0.95),  # 5% churn
            'avg_lifetime_months': 1 / (1 - 0.95),  # ~20 months
            'ltv': UNIT_ECONOMICS['ltv']['arpu'] * (1 / (1 - 0.95)) * costs['gross_margin']
        }
        ...
        cac_analysis = {
            ...
            'ltv_cac_ratio': customer_economics['ltv'] / UNIT_ECONOMICS['cac']['blended'],
            'payback_period_months': UNIT_ECONOMICS['cac']['blended'] / customer_economics['gross_profit_per_user'],
            'magic_number': customer_economics['gross_profit_per_user'] * 12 / UNIT_ECONOMICS['cac']['blended']
        }
        ...
        breakeven = {
            ...
            'users_needed_for_breakeven': int(costs['operating_costs']['total_monthly'] / customer_economics['gross_profit_per_user']),
            'mrr_needed': costs['operating_costs']['total_monthly'] / costs['gross_margin']
        }
        ...
        validation = {
            ...
            'health_check': {
                'ltv_cac_ratio': {
                    ...
                    'target': 3.0,
                    'status': 'Excellent' if cac_analysis['ltv_cac_ratio'] > 3 else 'Needs Improvement'
                },
                'payback_period': {
                    'value': cac_analysis['payback_period_months'],
                    'target': 12,  # < 12 months
                    'status': 'Excellent' if cac_analysis['payback_period_months'] < 12 else 'Needs Improvement'
                },
                'gross_margin': {
                    'value': costs['gross_margin'],
                    'target': 0.70,  # >70%
                    'status': 'Excellent' if costs['gross_margin'] > 0.70 else 'Good' if costs['gross_margin'] > 0.60 else 'Poor'
                }
            }
        }
        
        print(f"✅ Unit economics validated")
        print(f"   LTV/CAC Ratio: {cac_analysis['ltv_cac_ratio']:.1f}x ({validation['health_check']['ltv_cac_ratio']['status']})")
        print(f"   Payback Period: {cac_analysis['payback_period_months']:.1f} months ({validation['health_check']['payback_period']['status']})")
        print(f"   Gross Margin: {costs['gross_margin']*100:.1f}% ({validation['health_check']['gross_margin']['status']})")
        print(f"   Break-even: {breakeven['users_needed_for_breakeven']:,} paying users")
    ...
    def generate_pricing_recommendation(self) -> str:
        ...
        report += "-" * 80 + "\n"
        avg_competitor = self.competitor_pricing[self.competitor_pricing['price_monthly'] > 0]['price_monthly'].mean()
        ...
        report += f"   • Vertical specialization (research synthesis) vs horizontal tools\n"
        ...
        report += "   • Trigger: When user hits 50 document limit or needs citation export\n"
        report += "\n"
