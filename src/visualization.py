...
class GTMVisualizer:
    ...
    def plot_financial_projections(self, projections_df: pd.DataFrame) -> go.Figure:
        ...
        fig.add_trace(
            go.Scatter(
                x=projections_df["month"],
                y=projections_df["paying_users"],
                name="Paying Users",
                line=dict(color=self.colors["accent"], width=2.5),
                legend="legend",
            ),
            row=1,
            col=1,
        ...
        fig.add_trace(
            go.Scatter(
                x=projections_df["month"],
                y=projections_df["ltv_cac_ratio"],
                name="LTV/CAC Ratio",
                line=dict(color=self.colors["accent"], width=2.5),
                legend="legend3",
            ),
        ...
        fig.update_layout(
            ...
            # Legend 1
            legend=dict(
                x=0.365, y=1.0, xanchor="left", yanchor="top", bgcolor="rgba(0,0,0,0)"
            ),
            # Legend 2 - Updated to 0.925
            legend2=dict(
                x=0.925, y=1.0, xanchor="left", yanchor="top", bgcolor="rgba(0,0,0,0)"
            ),
            # Legend 3
            legend3=dict(
                x=0.365, y=0.44, xanchor="left", yanchor="top", bgcolor="rgba(0,0,0,0)"
            ),
            # Legend 4 - Updated to 0.925
            legend4=dict(
                x=0.925, y=0.44, xanchor="left", yanchor="top", bgcolor="rgba(0,0,0,0)"
            ),
        )
    ...
    def plot_channel_mix(self, channel_data: Dict) -> go.Figure:
        ...
        fig.update_layout(
            ...
            font=dict(family=CHART_STYLE["font_family"]),
            annotations=[
                dict(text="Total<br>Users", x=0.5, y=0.5, font_size=20, showarrow=False)
            ],
        )
    ...
    def plot_pricing_comparison(self, pricing_data: pd.DataFrame) -> go.Figure:
        ...
        competitor_prices = (
            paid_tiers.groupby("competitor_name")["price_monthly"].min().reset_index()
        )
        competitor_prices = competitor_prices.sort_values("price_monthly")

        # Highlight our product
        colors = [
            self.colors["accent"] if "Research" in name else self.colors["primary"]
        ...
        fig = go.Figure(
            data=[
                go.Bar(
                    ...
                    orientation="h",
                    marker_color=colors,
                    text=competitor_prices["price_monthly"].apply(
                        lambda x: f"${x:.0f}/mo"
                    ),
...
if __name__ == "__main__":
    ...
    try:
        ...
        financial_projections = pd.read_csv(
